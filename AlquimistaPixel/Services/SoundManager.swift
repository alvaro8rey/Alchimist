...
final class SoundManager {
    ...
    private init() {
        ...
        guard let fmt = AVAudioFormat(standardFormatWithSampleRate: SoundManager.sr, channels: 1) else { return }
    ...
    private static func makeBuffer(duration: Double) -> AVAudioPCMBuffer? {
        guard let fmt = AVAudioFormat(standardFormatWithSampleRate: sr, channels: 1) else { return nil }
