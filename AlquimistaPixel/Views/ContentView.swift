import SwiftUI
import SwiftData

struct ContentView: View {
    @Environment(\.modelContext) private var modelContext
    @Query private var discoveredElements: [DiscoveredElement]

    @StateObject private var vm = CanvasViewModel()
    @StateObject private var network = NetworkMonitor.shared
    @State private var screenSize: CGSize = .zero
    @State private var lastAddedElementName: String? = nil
    @State private var showOnboarding: Bool = {
        let username = UserDefaults.standard.string(forKey: "username") ?? ""
        let isGuest = UserDefaults.standard.bool(forKey: "isGuest")
        return username.isEmpty && !isGuest
    }()
    @AppStorage("username") private var storedUsername: String = ""
    @AppStorage("tutorialDone") private var tutorialDone = false
    @State private var showTutorial = false

    var body: some View {
        GeometryReader { geo in
            ZStack(alignment: .bottom) {
                CanvasView(vm: vm, screenSize: $screenSize, isOffline: !network.isConnected)

                InventoryView(
                    discoveredElements: discoveredElements,
                    activeElements: vm.activeElements,
                    newlyAddedName: lastAddedElementName,
                    onSpawn: { element in
                        let centerScreenX = screenSize.width / 2
                        let centerScreenY = screenSize.height / 2 - 80

                        let worldX = (centerScreenX - screenSize.width / 2 - vm.canvasOffset.width) / vm.scale
                        let worldY = (centerScreenY - screenSize.height / 2 - vm.canvasOffset.height) / vm.scale

                        vm.spawnElement(from: element, at: CGPoint(x: worldX, y: worldY))
                    }
                )

                // Toast de primer descubrimiento mundial
                if let name = vm.firstDiscoveryName {
                    VStack {
                        Spacer()
                        HStack(spacing: 10) {
                            Text("🌟")
                                .font(.title2)
                            VStack(alignment: .leading, spacing: 2) {
                                Text("¡Primer descubrimiento mundial!")
                                    .font(.system(size: 12, weight: .semibold))
                                    .foregroundStyle(.white.opacity(0.7))
                                Text(name)
                                    .font(.system(size: 16, weight: .black, design: .rounded))
                                    .foregroundStyle(.white)
                            }
                        }
                        .padding(.horizontal, 20)
                        .padding(.vertical, 14)
                        .background(.ultraThinMaterial)
                        .clipShape(Capsule())
                        .shadow(color: .white.opacity(0.15), radius: 20)
                        .padding(.bottom, 110)
                        .transition(.move(edge: .bottom).combined(with: .opacity))
                    }
                    .animation(.spring(response: 0.4, dampingFraction: 0.75), value: vm.firstDiscoveryName)
                }
            }
            .onAppear {
                screenSize = geo.size
                ensureBasicElements()

                // Si no hay sesión, decidir entre mostrar auth (online) o entrar como invitado (offline)
                if showOnboarding {
                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) {
                        if !network.isConnected {
                            UserService().enterGuestMode()
                            showOnboarding = false
                            if !tutorialDone {
                                DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                                    showTutorial = true
                                }
                            }
                        }
                    }
                }

                if !tutorialDone && !showOnboarding {
                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.6) {
                        showTutorial = true
                    }
                }

                // ESCUCHADOR DE DESCUBRIMIENTOS
                vm.onNewDiscovery = { name, emoji, hex, creatorName in
                    let exists = discoveredElements.contains { $0.name.lowercased() == name.lowercased() }

                    if !exists {
                        print("Nuevo descubrimiento detectado: \(name). Guardando en inventario...")
                        let new = DiscoveredElement(name: name, emoji: emoji, colorHex: hex)
                        new.creatorName = creatorName
                        modelContext.insert(new)
                        lastAddedElementName = name
                        Task {
                            try? await Task.sleep(nanoseconds: 50_000_000)
                            await MainActor.run { lastAddedElementName = nil }
                        }

                        do {
                            try modelContext.save()
                        } catch {
                            print("Error SwiftData: \(error)")
                        }

                        // Sincronizar con Firestore (solo para usuarios registrados)
                        Task {
                            await UserService().saveToInventory(
                                name: name, emoji: emoji, colorHex: hex, creatorName: creatorName
                            )
                        }
                    }
                }
            }
            .onChange(of: geo.size) { _, newSize in
                screenSize = newSize
                vm.resetCamera(screenSize: newSize)
            }
            // Al recuperar conexión siendo usuario registrado → sincronizar inventario local
            .onChange(of: network.isConnected) { _, isConnected in
                guard isConnected, !storedUsername.isEmpty else { return }
                Task { await syncInventoryToFirebase() }
            }
            // Al registrarse o iniciar sesión → sincronizar inventario local (cubre progreso offline)
            .onChange(of: storedUsername) { _, newUsername in
                guard !newUsername.isEmpty else { return }
                Task { await syncInventoryToFirebase() }
            }
            .animation(.spring(response: 0.4, dampingFraction: 0.8), value: network.isConnected)
        }
        .overlay {
            if showTutorial {
                TutorialView(isPresented: $showTutorial)
                    .transition(.opacity)
                    .zIndex(100)
            }
        }
        .animation(.easeInOut(duration: 0.3), value: showTutorial)
        .preferredColorScheme(.dark)
        .fullScreenCover(isPresented: $showOnboarding) {
            AuthView { inventoryItems in
                for item in inventoryItems {
                    let exists = discoveredElements.contains { $0.name.lowercased() == item.name.lowercased() }
                    if !exists {
                        let de = DiscoveredElement(name: item.name, emoji: item.emoji, colorHex: item.colorHex, discoveryDate: item.discoveryDate ?? Date())
                        de.creatorName = item.creatorName
                        modelContext.insert(de)
                    }
                }
                try? modelContext.save()
                showOnboarding = false
                if !tutorialDone {
                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.4) {
                        showTutorial = true
                    }
                }
            }
        }
    }

    private func syncInventoryToFirebase() async {
        let userService = UserService()
        for element in discoveredElements {
            await userService.saveToInventory(
                name: element.name,
                emoji: element.emoji,
                colorHex: element.colorHex,
                creatorName: element.creatorName ?? ""
            )
        }
    }

    private func ensureBasicElements() {
        let basics = [
            ("Fuego",  "🔥", "#FF5722"),
            ("Agua",   "💧", "#2196F3"),
            ("Tierra", "🌍", "#8B4513"),
            ("Aire",   "💨", "#E0E0E0")
        ]

        for (name, emoji, hex) in basics {
            if !discoveredElements.contains(where: { $0.name == name }) {
                let de = DiscoveredElement(name: name, emoji: emoji, colorHex: hex)
                modelContext.insert(de)
            }
        }
        try? modelContext.save()
    }
}
