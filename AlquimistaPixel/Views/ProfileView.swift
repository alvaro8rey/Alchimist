...
struct ProfileView: View {
    ...
    @AppStorage("isGuest") private var storedIsGuest: Bool = false
    private let userService = UserService()

    private var isGuest: Bool { userService.isGuest || userService.username.isEmpty }
        ...
        NavigationStack {
            ScrollView {
                VStack(spacing: 28) {
                    ...
                    if !isGuest { accountActionsSection }
                }
                .padding(.bottom, 48)
            }
            .background(bg)
        ...
        .fullScreenCover(isPresented: $showAuth) {
            AuthView { inventoryItems in
                for item in inventoryItems {
                    if !elements.contains(where: { $0.name.lowercased() == item.name.lowercased() }) {
    ...
    private var avatarSection: some View {
        VStack(spacing: 10) {
            ...
            Text(isGuest ? "Invitado" : username)
                .font(.system(size: 26, weight: .black, design: .rounded))
                .foregroundStyle(isGuest ? .white.opacity(0.45) : .white)

            HStack(spacing: 5) {
                Image(systemName: isGuest ? "person.slash" : "flask.fill")
                    .font(.system(size: 10))
                    .foregroundStyle(.white.opacity(0.3))
                Text(isGuest ? "Sin cuenta" : "Alquimista")
    ...
    private var accountActionsSection: some View {
        VStack(spacing: 10) {
            ...
            Button {
                ...
                HStack(spacing: 8) {
                    if isDeleting {
                        ProgressView().tint(.red.opacity(0.8))
                    } else {
        ...
        .alert("Eliminar cuenta", isPresented: $showDeleteAlert) {
            ...
            Text("Se eliminarán tu cuenta y todos tus datos de forma permanente. Esta acción no se puede deshacer.")
        }
    }
    ...
    private func performDeleteAccount() async {
        ...
        do {
            ...
            // Borrar inventario local
            for element in elements { modelContext.delete(element) }
            try? modelContext.save()
            storedUsername = ""
            storedIsGuest = false
